...
set _DeploySetParametersFile=%RootPath%Muse.SetParameters.xml
)
...
@rem ---------------------------------------------------------------------------------
if not exist "%RootPath%Muse.zip" (
echo "%RootPath%Muse.zip" does not exist. 
...
set _MSDeployCommandline="%MSDeployPath%msdeploy.exe" -source:package='%RootPath%Muse.zip',EncryptPassword='Encryption password is used only if any deployment setting is marked as secure' -dest:%_Destination%,EncryptPassword='Encryption password is used only if any deployment setting is marked as secure' -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension
) else (
set _MSDeployCommandline="%MSDeployPath%msdeploy.exe" -source:package='%RootPath%Muse.zip',EncryptPassword='Encryption password is used only if any deployment setting is marked as secure' -dest:%_Destination%,EncryptPassword='Encryption password is used only if any deployment setting is marked as secure' -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -setParamFile:"%_DeploySetParametersFile%"
)
...
:usage
echo =========================================================
if not exist "%RootPath%Muse.deploy-readme.txt" (
...
start notepad "%RootPath%Muse.deploy-readme.txt"
)
echo =========================================================
goto :eof
